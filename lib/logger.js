...
function ColoredConsoleLogger(defaultContext)
{
    LoggerBase.call(this, defaultContext);
    this.colors = {};
    this.colors[this.ERROR] = cursor.red;
    this.colors[this.WARNING] = cursor.yellow;
    this.colors[this.INFO] = cursor.green;
    this.colors[this.DEBUG] = cursor.grey;

    this.colors['*'] = cursor.grey;
}
...
ColoredConsoleLogger.prototype.__log = function (me, level, vars, args)
{
    if (args == null && vars != null) { args = vars; vars = null; }
    ...
    cursor.cyan().write('').write(dateformat(vars.when, this.dateFormat)).write('').grey().write(' [');
    var fn = me.colors[level] || me.colors['*'];
    fn.call(cursor);
    cursor.write(lev).grey().write('] ').grey().write(ctx).write(' ').reset().write(args.map(nice).join(" ")).reset().write("\n");
